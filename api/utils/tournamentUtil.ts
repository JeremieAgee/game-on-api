...
const createTournament = async (req: Request, res: Response) => {
    ...
    const currentTournament = { name, game, platform, prize, startDate, startTime }
    const { data, error } = await supabaseDB.post("/tournaments", currentTournament)
    if (error) {
...
const updateTournament = async (req:Request, res:Response) => {
    ...
    const { data, error }= await supabaseDB.put("/tournaments", id, newTournament)
